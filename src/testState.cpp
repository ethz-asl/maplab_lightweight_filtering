...
class StateTesting2 : public virtual ::testing::Test {
 protected:
  ...
  static const unsigned int V_ = 3;
  static const unsigned int Q_ = 2;
  typedef LWF::AugmentedState<LWF::ComposedState2<LWF::StateArray<LWF::ScalarState,S_>,LWF::StateArray<LWF::VectorState<3>,V_>,LWF::StateArray<LWF::QuaternionState,Q_>>,Auxillary> StateSVQAugmented;
  StateSVQAugmented testState1_;
  StateSVQAugmented testState2_;
  StateSVQAugmented::mtDifVec difVec_;
  double testScalar1_[S_];
...
//  ASSERT_TRUE(testState1_.getValue<q1>().isNear(testQuat1_[1],1e-6));
  ...
  ASSERT_EQ(testState1_.getId<s0>(),0);
  ASSERT_EQ(testState1_.getId<s1>(),1);
  ASSERT_EQ(testState1_.getId<s2>(),2);
  ASSERT_EQ(testState1_.getId<s3>(),3);
  ASSERT_EQ(testState1_.getId<v0>(),4);
  ASSERT_EQ(testState1_.getId<v1>(),7);
  ASSERT_EQ(testState1_.getId<v2>(),10);
  ASSERT_EQ(testState1_.getId<q0>(),13);
  ASSERT_EQ(testState1_.getId<q1>(),16);
