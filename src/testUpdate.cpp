...
//// Test predictEKF
//TEST_F(UpdateModelTest, predictEKF) {
//  testUpdate_.setMeasurement(testMeas_);
//  UpdateExample::mtState::mtCovMat cov;
//  cov.setIdentity();
//  UpdateExample::mtJacInput F = testUpdate_.jacInput(testState_,testMeas_,dt_);
//  UpdateExample::mtJacNoise Fn = testUpdate_.jacNoise(testState_,testMeas_,dt_);
//  UpdateExample::mtState::mtCovMat predictedCov = F*cov*F.transpose() + Fn*testUpdate_.prenoiP_*Fn.transpose();
//  UpdateExample::mtState state;
//  state = testState_;
//  testUpdate_.predictEKF(state,cov,dt_);
//  UpdateExample::mtState::mtDiffVec dif;
//  UpdateExample::mtNoise noise;
//  state.boxMinus(testUpdate_.eval(testState_,testMeas_,noise,dt_),dif);
//  ASSERT_NEAR(dif.norm(),0.0,1e-6);
